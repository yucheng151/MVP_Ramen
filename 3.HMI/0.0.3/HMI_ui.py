...
from config import (
    HEARTBEAT_INTERVAL,
    PLC_DEBUG_LOG_DIR,
    PLC_DEBUG_LOG_HEARTBEAT_SECONDS,
    PLC_DEBUG_LOG_RETENTION_DAYS,
    PLC_IP,
    PLC_PORT,
    RECONNECT_DELAY,
)
...
from ui_common import BG, configure_dark_theme
...
class HMIUI:
    def __init__(
        ...
        self.plc_debug_log = PLCDebugLog(
            PLC_DEBUG_LOG_DIR,
            retention_days=PLC_DEBUG_LOG_RETENTION_DAYS,
            heartbeat_seconds=PLC_DEBUG_LOG_HEARTBEAT_SECONDS,
        )
    ...
    def _schedule_font_resize(self, event=None) -> None:
        ...
        # Resize events arrive continuously while dragging.  Wait for the
        # window to settle so hundreds of widgets are not reconfigured per
        # mouse movement.
        self._font_resize_job = self.root.after(260, self._apply_responsive_fonts)

    def _apply_responsive_fonts(self, force=False) -> None:
        self._font_resize_job = None
        ...
        scale = (
            ...
            if window_ratio <= 1.0
            else min(1.28, 1.0 + ((window_ratio - 1.0) * 0.65))
        )
        # Ten-percent buckets prevent tiny geometry changes from triggering a
        # complete font/layout pass.
        scale = round(scale * 10) / 10
        if not force and scale == self._last_font_scale:
            return
        self._last_font_scale = scale
        current_page = self.pages.get(getattr(self, "current_page", ""))
        for key, font_data in list(self._responsive_fonts.items()):
            ...
            try:
                if not widget.winfo_exists():
                    ...
                    continue
                # Hidden pages are scaled when they are opened.  Skipping them
                # here removes most resize-time layout work.
                if current_page is not None:
                    owner = widget
                    while owner is not None and owner is not current_page:
    ...
    def show_page(self, name: str) -> None:
        ...
        self.root.after_idle(lambda: self._apply_responsive_fonts(force=True))
    ...
    def _poll_loop(self) -> None:
        while not self._stop_event.is_set():
            ...
            debug_words = (
                None if self.mock_mode else self.plc.read_d(DEBUG_START, DEBUG_COUNT)
            )
            monitor_words = (
                None if self.mock_mode else self.plc.read_d(MONITOR_START, MONITOR_COUNT)
            )
            if not self.mock_mode and mode_data is not None:
                self.plc_debug_log.capture_blocks(
                    debug_words,
                    monitor_words,
                    auto_enabled=(
                        int(mode_data[0]) == MACHINE_MODE_AUTO
                        or self._last_valid_machine_mode == MACHINE_MODE_AUTO
                    ),
                    machine_mode_raw=mode_data[0],
                )
            auto_live = (
                None
                if self.mock_mode or mode_data is None
                else self.auto_live_monitor.read_snapshot(
                    mode_value=mode_data[0],
                    debug_block=debug_words,
                    monitor_block=monitor_words,
                )
            )
