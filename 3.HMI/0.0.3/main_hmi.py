...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    parser.add_argument(
        "--profile", choices=("simulation", "field"), default="simulation",
        help="HMI版本：simulation允許測試旁路；field啟用現場安全檢查",
    )
    parser.add_argument(
        "--page", default="MainPage",
        choices=("MainPage", "AutoSystemPage"),
        help="HMI啟動後首先顯示的頁面",
    )
    args = parser.parse_args(argv)

    if args.profile == "field" and args.mock:
        parser.error("FIELD現場版禁止使用 --mock")
    ...
    ui = HMIUI(
        ip=args.ip, port=args.port, mock=args.mock,
