...
class ConveyorControlPage(tk.Frame):
    ...
    def _build_manual(self, parent):
        ...
        self.semi_auto_frame = tk.Frame(
            frame, bg="#162832", highlightbackground=BLUE, highlightthickness=1,
        )
        tk.Label(
            self.semi_auto_frame, text="半自動測試", bg="#162832", fg=TEXT,
            font=("Microsoft JhengHei UI", 13, "bold"),
        ).pack(anchor="w", padx=14, pady=(9, 4))
        choices = tk.Frame(self.semi_auto_frame, bg="#162832")
        choices.pack(fill="x", padx=10)
        self.semi_auto_actions = {}
        for action in ("落碗", "煮麵", "前三料", "後三料", "加湯"):
            selected = tk.BooleanVar(value=False)
            check = tk.Checkbutton(
                choices, text=action, variable=selected, bg="#162832", fg=TEXT,
                activebackground="#162832", activeforeground=TEXT,
                selectcolor=PANEL, font=("Microsoft JhengHei UI", 10),
            )
            check.pack(side="left", expand=True, padx=3, pady=3)
            self.semi_auto_actions[action] = selected
        self.execute_semi_auto_button = tk.Button(
            self.semi_auto_frame, text="執行選取動作",
            command=self._execute_semi_auto_actions, **button_style(BLUE),
        )
        self.execute_semi_auto_button.pack(fill="x", padx=14, pady=(5, 10))

    def _clear_semi_auto_selection(self):
        """Clear local UI state only; this must never write to the PLC."""
        for selected in self.semi_auto_actions.values():
            selected.set(False)

    def _set_semi_auto_visible(self, visible):
        if visible == self._semi_auto_visible:
            return
        self._semi_auto_visible = visible
        if visible:
            self.semi_auto_frame.pack(fill="x", padx=18, pady=(0, 12))
        else:
            self.semi_auto_frame.pack_forget()
            self._clear_semi_auto_selection()

    def _execute_semi_auto_actions(self):
        # Re-check the PLC-reported value at click time, not merely widget state.
        if self.app.snapshot.get("machine_mode") != 1:
            self._set_semi_auto_visible(False)
            return
        actions = [
            name for name, selected in self.semi_auto_actions.items()
            if selected.get()
        ]
        if not actions:
            messagebox.showwarning("半自動測試", "請至少選取一個動作", parent=self)
            return
        handler = getattr(self.app, "execute_semi_auto_actions", None)
        if handler is None:
            messagebox.showwarning(
                "半自動測試", "PLC 尚未提供半自動多動作命令介面", parent=self,
            )
            return
        ok, message = handler(actions)
        (messagebox.showinfo if ok else messagebox.showwarning)(
            "半自動測試", message, parent=self,
        )
    ...
    def refresh(self):
        ...
        self._set_semi_auto_visible(snapshot.get("machine_mode") == 1)
        ...
        self.execute_semi_auto_button.configure(
            state="normal" if snapshot.get("online", False) else "disabled"
        )
        word = data[0]
        faults = [name for bit, name in enumerate(FAULT_NAMES) if word & (1 << bit)]
