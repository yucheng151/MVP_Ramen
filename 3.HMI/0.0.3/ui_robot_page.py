...
class RobotPage(BasePage):
    ...
    def _robot_interlock_reasons(self):
        ...
        external_bypass = snapshot.get("external_devices_bypassed", False)
        reasons = []
        ...
        if not external_bypass and (robot is None or not robot.read_ok or not robot.busy):
            reasons.append("Robot is Offline")
        if not external_bypass and (manual_reply is None or not manual_reply.read_ok):
            reasons.append("Robot manual status communication is offline")
        elif not external_bypass and manual_reply.robot_idle is not True:
    ...
    def _update_robot_command_state(self):
        ...
        if not snapshot.get("heartbeat_ok"):
            ...
            return
        robot = snapshot.get("robot")
        if (not snapshot.get("external_devices_bypassed")
                and (robot is None or not robot.read_ok or not robot.busy)):
            self._finish_robot_command(
    ...
    def refresh(self):
        ...
        robot = snapshot.get("robot")
        manual_reply = snapshot.get("robot_manual")
        robot_alarm = False if snapshot.get("external_devices_bypassed") else bool(
            robot is not None
            and robot.read_ok
            and (
                robot.error_signal
        ...
        idle = (
            True
            if snapshot.get("external_devices_bypassed")
            else
        ...
        if self.app.snapshot.get("external_devices_bypassed"):
            self._comm_value.configure(text="SIMULATION PASS", fg=GREEN)
            for value, _warning in self._bit_values.values():
                value.configure(text="BYPASS", fg=GREEN)
