...
class SideNavigation(tk.Frame):
    ...
    def __init__(self, parent, app):
        ...
        nav_items = (
            ...
            ("AUTO SYSTEM", "AUT", "Auto", "AutoSystemPage"),
    ...
    def refresh(self):
        ...
        robot_alarm = (
            False if snapshot.get("external_devices_bypassed") else bool(
            (
                robot is not None
...
class MainPage(tk.Frame):
    ...
    def _schedule_render(self, _event=None):
        if getattr(self.app, "current_page", None) != "MainPage":
            return
        if self._resize_job:
            self.after_cancel(self._resize_job)
        self._resize_job = self.after(180, self._render_scene)
    ...
    def _render_scene(self):
        ...
        if render_size == self._last_render_size and self._photo is not None:
            return
