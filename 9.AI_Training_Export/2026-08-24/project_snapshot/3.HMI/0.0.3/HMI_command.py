...
class HMICommand:
    ...
    def next_command_index(self) -> int:
        """從 D1001 與 PLC ACK 接續下一個 Index，並在 0~65535 間循環。

        HMI 重新啟動後，Python 物件內的 ``last_command_index`` 會回到 0，
        但 PLC 仍可能保留上一筆命令編號。每次送命令前先讀 D1001 與
        D1102，可避免重複使用舊 Index 而讓 PLC 把新命令判斷成上一筆。
        """
        if self.plc.connected:
            hmi_value = self.plc.read_d(D_HMI_TO_PLC_CMD_INDEX, 1)
            ack_value = self.plc.read_d(PLC_CMD_ACK_INDEX, 1)
            candidates = [self.last_command_index]
            if hmi_value:
                candidates.append(int(hmi_value[0]) & 0xFFFF)
            if ack_value:
                candidates.append(int(ack_value[0]) & 0xFFFF)

            # 16-bit 序號比較：差值小於半圈者視為較新，可正確跨越 65535→0。
            newest = candidates[0]
            for candidate in candidates[1:]:
                delta = (candidate - newest) & 0xFFFF
                if 0 < delta < 0x8000:
                    newest = candidate
            self.last_command_index = newest
