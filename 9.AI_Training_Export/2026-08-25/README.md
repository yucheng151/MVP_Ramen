...
| `corpus/session_delta_2026-08-25.md` | 2026-08-23至25最後確認的三麵篩、煮麵計時、仲裁與HMI耐久測試增量 |
...
| `prompts/CLAUDE_HANDOFF_PROMPT.md` | 可直接貼給Claude的專案接手提示詞與閱讀順序 |
| `reference_logic/` | 從使用者最新版程式整理的候選ST參考碼；匯入前仍須ISPSoft編譯核對 |
| `manifests/authoritative_sources.md` | 版本真值、來源優先級與衝突裁決規則 |
...
| `manifests/package_file_manifest.csv` | 整個交接知識包的檔案大小與SHA-256 |
| `test_evidence/` | V101 1000筆PASS與先前304筆逾時FAIL的summary、log、CSV |
| `build_project_export.ps1` | 可重新建立來源快照與manifest的匯出腳本 |

## 建議使用順序

1. Claude先讀 `prompts/CLAUDE_HANDOFF_PROMPT.md`。
2. 再讀 `corpus/session_delta_2026-08-25.md`、`project_knowledge.md`及`plc_control_logic_memory.md`。
3. 遇到版本矛盾時依 `manifests/authoritative_sources.md` 裁決，不得自行拼接V100與V101。
4. 用 `evaluation_cases.jsonl` 測試模型；未達安全與正確性門檻不得生成上機版本。
5. 經人工逐筆審查後，才能把 `instruction_qa.jsonl` 用於微調。
6. PLC輸出仍須經ISPSoft編譯、Simulator、I/O測試及合格工程師審查。

## 重要版本狀態

- `1.PLC/MVP_V2_101`是後續執行1000筆AS200耐久測試的版本；`MVP_V2_100`保留前一版與部分重新匯出PDF供差異對照。兩版不得混用，現場正式版仍須實機比對。
...
快照包含目前兩個需對照的PLC版本`MVP_V2_100`與`MVP_V2_101`，以及電路圖、HMI `0.0.3`、I/O、Nachi Robot、IPC `0.0.1/0.0.2`、Ordering `0.0.1`及全部測試程式。`0.Old`、其他PLC歷史版本、`tmp`、cache、log、Python bytecode及自動 `.~bak` 不複製進訓練快照；歷史內容仍留在原專案供追溯。
