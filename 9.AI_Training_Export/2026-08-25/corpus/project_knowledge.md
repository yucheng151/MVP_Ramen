...
- `MVP_V2_101`：後續執行1000筆AS200耐久測試的版本；該次結果1000/1000 PASS，仍不等同實機安全驗收。
- `MVP_V2_100`：前一版與部分最新PDF來源；不得假設其後續修改已同步到V101。
...
## 13. 2026-08-25三麵篩與煮麵計時補充

- 三個麵篩可同時煮麵；Nachi拿生麵動作仍一次一筆。
- 最新Scheduler定義Firmness：1硬、2正常、3軟，分別選擇Hard/Normal/Soft時間。
- `Noodlebasket PRG`以T8/T9/T10獨立計時，S2來自各麵篩`CookTimeSet`，Done後把State由40改為50。
- 站2對應麵篩正在State 40時，Arbiter應允許Nachi裝填其他State 20麵篩，否則三麵篩會被不必要地序列化。
- 詳細版本差異與回歸要求見`session_delta_2026-08-25.md`。
